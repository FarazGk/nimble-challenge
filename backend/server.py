...
async def handle_signaling(pc, signaling, recorder):
    ...
    while True:
        ...
        debug_print(
            f'[SERVER] receive obj RTCSessionDescription: {isinstance(obj, RTCSessionDescription)}, IceCandidate: {isinstance(obj, RTCIceCandidate)} ')
...
async def server_handler(pc, signaling, recorder):
    ...
    def on_message(message):
        ...
        if isinstance(message, str) and message.startswith('value'):
            ...
            pr_green(
                f'[RESULT] Prediction Position: ({predictedX}, {predictedY}), Real Position: ({ball_track.x}, {ball_track.y}), Error: ({pos_error[0]}, {pos_error[1]})')
            msg = f'image {current_stamp()}'
            debug_print(f'[CHANNEL] Send Message: {msg}', 1)
            channel.send(msg)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--signaling_host', type=str, default='0.0.0.0', help='host')
    parser.add_argument('--signaling_port', type=str, default='5001', help='port')
