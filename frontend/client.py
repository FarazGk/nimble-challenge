...
def current_stamp():
    ...
    minus the first request time stamp
    """
    global time_start
...
async def handle_signaling(pc, signaling):
    ...
    while True:
        ...
        debug_print(
            f'[CLIENT] Receive obj RTCSessionDescription: {isinstance(obj, RTCSessionDescription)}, IceCandidate: {isinstance(obj, RTCIceCandidate)} ')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--signaling_host', type=str, default='backend-service', help='host')
    ...
    ballPosX = mp.Value('d', 2.0)
    ballPosY = mp.Value('d', 1.0)
